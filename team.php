...
function setup_team_record() {
    ...
    $totalTies = 0;
    //Guest Wins
    $result = mysql_query($gwinsSelect, $Link) or die("sp_clubs (Line " . __LINE__ . "): " . mysql_errno() . ": " . mysql_error());
    if ($result && mysql_num_rows($result) > 0) {
        $gwin = mysql_fetch_assoc($result);
        $totalWins+= $gwin['gwins'];
    }
    //Home Wins
    $result = mysql_query($hwinsSelect, $Link) or die("sp_clubs (Line " . __LINE__ . "): " . mysql_errno() . ": " . mysql_error());
    if ($result && mysql_num_rows($result) > 0) {
        $hwin = mysql_fetch_assoc($result);
        $totalWins+= $hwin['hwins'];
    }
    //Guest Losses
    $result = mysql_query($glossesSelect, $Link) or die("sp_clubs (Line " . __LINE__ . "): " . mysql_errno() . ": " . mysql_error());
    if ($result && mysql_num_rows($result) > 0) {
        $gloss = mysql_fetch_assoc($result);
        $totalLosses+= $gloss['glosses'];
    }
    //Home Losses
    $result = mysql_query($hlossesSelect, $Link) or die("sp_clubs (Line " . __LINE__ . "): " . mysql_errno() . ": " . mysql_error());
    if ($result && mysql_num_rows($result) > 0) {
        $hloss = mysql_fetch_assoc($result);
        $totalLosses+= $hloss['hlosses'];
    }
    //Guest Losses
    $result = mysql_query($gtiesSelect, $Link) or die("sp_clubs (Line " . __LINE__ . "): " . mysql_errno() . ": " . mysql_error());
    if ($result && mysql_num_rows($result) > 0) {
        $gtie = mysql_fetch_assoc($result);
        $totalTies+= $gtie['gties'];
    }
    //Home Losses
    $result = mysql_query($htiesSelect, $Link) or die("sp_clubs (Line " . __LINE__ . "): " . mysql_errno() . ": " . mysql_error());
    if ($result && mysql_num_rows($result) > 0) {
