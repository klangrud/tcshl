...
function game_has_stats($gameid) {
    ...
    $ttlStats = 0;
    // Goalie stat count
    $gStatResult = mysql_query($gStatSelect, $Link) or die("sp_clubs (Line " . __LINE__ . "): " . mysql_errno() . ": " . mysql_error());
    if ($gStatResult && mysql_num_rows($gStatResult) > 0) {
        $gStats = mysql_fetch_assoc($gStatResult);
        $ttlStats+= $gStats['count'];
    }
    // Player stat count
    $pStatResult = mysql_query($pStatSelect, $Link) or die("sp_clubs (Line " . __LINE__ . "): " . mysql_errno() . ": " . mysql_error());
