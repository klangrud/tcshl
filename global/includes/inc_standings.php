            ...
            if ($teamStatsResult && mysql_num_rows($teamStatsResult) > 0) {
                ...
                while ($game = mysql_fetch_array($teamStatsResult, MYSQL_ASSOC)) {
                    ...
                    if (is_home_team($tid, $game['gameHomeTeam'])) {
                        ...
                        // Add a loss
                        else if ($game['gameGuestScore'] > $game['gameHomeScore']) {
                            $losses++;
                        }
                        // Add a tie
                        else {
...
function compare_points($a, $b) {
    ...
    if (!$retval) $retval = strnatcmp($b['winningpercentage'], $a['winningpercentage']);
    if (!$retval) return strnatcmp($a['teamname'], $b['teamname']);
    return $retval;
}
?>
