...
$id = 0;
if ($_GET) {
    if (isset($_GET['id']) && $_GET['id'] > 0 && idCheck_paymentDB($_GET['id'], 2)) {
        $id = $_GET['id'];
    } else {
...
$P2_AUDIT = $_SESSION['userid'];
$P2_NAME = get_registrant_name($id);
$P2_ID = $id;
$P2_PAY1_PROCESS = '';
$P2_PAY1_DATE = '';
$P2_PAY1_DATE_DB = '';
...
if ((isset($_POST['action'])) && ($_POST['action'] == "Edit Payments")) {
    ...
    if ($errors = validate_payments_form()) {
        ...
        if ($errors = process_payments_form()) {
            ...
            header("Location: editpaymentplan2.php?id={$P2_ID}&success=yes");
        }
    }
} else {
...
format_date_fields();
$smarty->assign('P2_AUDIT', $P2_AUDIT);
$smarty->assign('P2_NAME', $P2_NAME);
$smarty->assign('P2_ID', $P2_ID);
$smarty->assign('P2_PAY1_PROCESS', $P2_PAY1_PROCESS);
$smarty->assign('P2_PAY1_DATE_SELECT', $P2_PAY1_DATE_SELECT);
$smarty->assign('P2_PAY1_DATE', $P2_PAY1_DATE);
$smarty->assign('P2_PAY1_DATE_DB', $P2_PAY1_DATE_DB);
$smarty->assign('P2_PAY1_CHECK', $P2_PAY1_CHECK);
$smarty->assign('P2_PAY2_PROCESS', $P2_PAY2_PROCESS);
$smarty->assign('P2_PAY2_DATE_SELECT', $P2_PAY2_DATE_SELECT);
$smarty->assign('P2_PAY2_DATE', $P2_PAY2_DATE);
$smarty->assign('P2_PAY2_DATE_DB', $P2_PAY2_DATE_DB);
$smarty->assign('P2_PAY2_CHECK', $P2_PAY2_CHECK);
// Build the page
require ('global_begin.php');
$smarty->display('admin/editpaymentplan2.tpl');
...
function populateFieldsFromDatabase() {
    ...
    global $P2_ID;
    global $P2_PAY1_DATE;
    global $P2_PAY1_DATE_DB;
    global $P2_PAY1_CHECK;
    global $P2_PAY2_DATE;
    global $P2_PAY2_DATE_DB;
    global $P2_PAY2_CHECK;
    $select = 'SELECT * FROM ' . PAYMENTPLANTWO . ' WHERE registrationID=' . $P2_ID;
    $result = mysql_query($select, $Link) or die("sp_clubs (Line " . __LINE__ . "): " . mysql_errno() . ": " . mysql_error());
    if ($result && mysql_num_rows($result) > 0) {
        $p2 = mysql_fetch_assoc($result);
        //$rName = $p2['fName'].' '.$p2['lName'];
        $P2_PAY1_DATE = $p2['paymentOneDate'];
        $P2_PAY1_DATE_DB = $p2['paymentOneDate'];
        $P2_PAY1_CHECK = $p2['p1_checknum'];
        $P2_PAY2_DATE = $p2['paymentTwoDate'];
        $P2_PAY2_DATE_DB = $p2['paymentTwoDate'];
        $P2_PAY2_CHECK = $p2['p2_checknum'];
    }
}
function format_date_fields() {
    global $P2_PAY1_DATE;
    global $P2_PAY2_DATE;
    global $P2_PAY1_DATE_SELECT;
    global $P2_PAY2_DATE_SELECT;
    $P2_PAY1_DATE_SELECT = select_payment_month('p2_pay1_month', $P2_PAY1_DATE) . select_payment_day('p2_pay1_day', $P2_PAY1_DATE) . select_payment_year('p2_pay1_year', $P2_PAY1_DATE);
    $P2_PAY2_DATE_SELECT = select_payment_month('p2_pay2_month', $P2_PAY2_DATE) . select_payment_day('p2_pay2_day', $P2_PAY2_DATE) . select_payment_year('p2_pay2_year', $P2_PAY2_DATE);
}
function handle_reposts() {
    ...
    global $P2_PAY2_DATE_DB;
    global $P2_PAY2_CHECK;
    $P2_PAY1_DATE = $_POST['p2_pay1_year'] . '-' . $_POST['p2_pay1_month'] . '-' . $_POST['p2_pay1_day'] . ' 00:00:00';
    $P2_PAY2_DATE = $_POST['p2_pay2_year'] . '-' . $_POST['p2_pay2_month'] . '-' . $_POST['p2_pay2_day'] . ' 00:00:00';
    $P2_PAY1_DATE_DB = $_POST['p2_pay1_date_db'];
    $P2_PAY2_DATE_DB = $_POST['p2_pay2_date_db'];
    $P2_PAY1_CHECK = $_POST['p1_checknum'];
    $P2_PAY2_CHECK = $_POST['p2_checknum'];
}
function initialize_payment_dates($season) {
    global $Link;
    global $PAYMENT_DATE_1;
    global $PAYMENT_DATE_3;
    $select = 'SELECT paymentOneDate, paymentThreeDate FROM ' . PAYMENTDATES . ' WHERE seasonID=' . $season;
    $result = mysql_query($select, $Link) or die("sp_clubs (Line " . __LINE__ . "): " . mysql_errno() . ": " . mysql_error());
    if ($result && mysql_num_rows($result) > 0) {
...
function validate_payments_form() {
    $errors = array();
    if (isset($_POST['update1']) && $_POST['update1'] == "YES" || isset($_POST['update2']) && $_POST['update2'] == "YES") {
